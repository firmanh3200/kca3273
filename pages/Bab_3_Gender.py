...
data = pd.read_excel("data/penduduk/jumlah_penduduk_jk.xlsx")
sort_data = data.sort_values(by=['tahun', 'semester', 'namakec', 'namakel'], ascending=[False,False,True,True])
pivot_tabel = sort_data.pivot_table(index=['namakec', 'namakel', 'tahun', 'semester'], columns='jenis_kelamin', values='jumlah_penduduk').reset_index()
pivot_tabel = pivot_tabel.sort_values(by=['tahun', 'semester'], ascending=False)        
datagender = sort_data.groupby(['namakec', 'jenis_kelamin', 'tahun', 'semester'])['jumlah_penduduk'].sum().reset_index()
...
with st.container(border=True):
    st.info(f"Jumlah Penduduk di Kecamatan {pilihkec} menurut Jenis Kelamin, Semester {pilihsem} Tahun {pilihtahun}")
    ...
    if pilihkec and pilihtahun and pilihsem:
        ...
        with kol1d:
            pie_kk = px.pie(tabel, values='jumlah_penduduk', names='jenis_kelamin', 
        ...
        with kol1e:
            bar_kk = px.bar(tabel, x='jenis_kelamin', y='jumlah_penduduk', color='jenis_kelamin', 
                            color_discrete_sequence=warna_options[pilihwarna])
            bar_kk.update_layout(showlegend=False)
            with st.container(border=True):
                st.plotly_chart(bar_kk, use_container_width=True)
        with kol1f:
            tabeljk = pivot_tabel[(pivot_tabel['namakec'] == pilihkec) & (pivot_tabel['tahun'] == pilihtahun) & (pivot_tabel['semester'] == pilihsem)]
            tabeljk['Jumlah'] = tabeljk['LAKI-LAKI'] + tabeljk['PEREMPUAN']
    ...
    area_kk = px.area(tren_kk, x='sem', y='jumlah_penduduk', color='jenis_kelamin', 
                            color_discrete_sequence=warna_options[pilihwarna])
    st.plotly_chart(area_kk, use_container_width=True)
    
    st.subheader("", divider='rainbow')
    barkk = px.bar(tren_kk, x='sem', y='jumlah_penduduk', color='jenis_kelamin', 
                            color_discrete_sequence=warna_options[pilihwarna])
    st.plotly_chart(barkk, use_container_width=True)
    
    st.subheader("", divider='rainbow')
    line_kk = px.line(tren_kk, x='sem', y='jumlah_penduduk', color='jenis_kelamin', 
                            color_discrete_sequence=warna_options[pilihwarna])
