...
data = pd.read_excel("data/penduduk/jumlah_penduduk_pekerjaan.xlsx")
sort_data = data.sort_values(by=['tahun', 'semester', 'namakec', 'namakel', 'jumlah_penduduk'], ascending=[False,False,True,True, False])
#pivot_tabel = sort_data.pivot_table(index=['namakec', 'namakel', 'tahun', 'semester'], columns='jenis_pekerjaan', values='jumlah_penduduk').reset_index()
#pivot_tabel = pivot_tabel.sort_values(by=['tahun', 'semester'], ascending=False)        
datagender = sort_data.groupby(['namakec', 'jenis_pekerjaan', 'tahun', 'semester'])['jumlah_penduduk'].sum().reset_index()
...
with st.container(border=True):
    st.info(f"Jumlah Penduduk di Kecamatan {pilihkec} menurut Jenis Pekerjaan, Semester {pilihsem} Tahun {pilihtahun}")
    ...
    if pilihkec and pilihtahun and pilihsem:
        ...
        with kol1d:
            pie_kk = px.pie(tabel, values='jumlah_penduduk', names='jenis_pekerjaan', 
        ...
        with kol1e:
            bar_kk = px.bar(tabel, x='jenis_pekerjaan', y='jumlah_penduduk', color='jenis_pekerjaan', 
                            color_discrete_sequence=warna_options[pilihwarna])
    ...
    with kol2a:
        trimep = px.treemap(dataterpilih, path=['namakel', 'jenis_pekerjaan'], 
                            values='jumlah_penduduk', color_discrete_sequence=warna_options[pilihwarna])
        with st.container(border=True):
            st.plotly_chart(trimep, use_container_width=True)
    with kol2b:
        bulet = px.sunburst(dataterpilih, path=['namakec', 'namakel', 'jenis_pekerjaan'], values='jumlah_penduduk')
        with st.container(border=True):
    ...
    area_kk = px.area(tren_kk, x='sem', y='jumlah_penduduk', color='jenis_pekerjaan', 
                            color_discrete_sequence=warna_options[pilihwarna])
    st.plotly_chart(area_kk, use_container_width=True)
    
    st.subheader("", divider='rainbow')
    barkk = px.bar(tren_kk, x='sem', y='jumlah_penduduk', color='jenis_pekerjaan', 
                            color_discrete_sequence=warna_options[pilihwarna])
    st.plotly_chart(barkk, use_container_width=True)
    
    st.subheader("", divider='rainbow')
    line_kk = px.line(tren_kk, x='sem', y='jumlah_penduduk', color='jenis_pekerjaan', 
                            color_discrete_sequence=warna_options[pilihwarna])
    st.plotly_chart(line_kk, use_container_width=True)
    
    
st.link_button("Sumber Data", url="https://opendata.bandung.go.id/dataset/jumlah-penduduk-kota-bandung-berdasarkan-jenis-pekerjaan")