...
data = pd.read_excel("data/penduduk/jumlah_penduduk_umur.xlsx")
data['kategori_umur'] = data['kategori_umur'].astype(str)
sort_data = data.sort_values(by=['tahun', 'semester', 'namakec'], ascending=[False,False,True])
pivot_tabel = sort_data.pivot_table(index=['namakec', 'tahun', 'semester'], columns='kategori_umur', values='jumlah_penduduk').reset_index()
pivot_tabel = pivot_tabel.sort_values(by=['tahun', 'semester'], ascending=False)        
datagender = sort_data.groupby(['namakec', 'kategori_umur', 'tahun', 'semester'])['jumlah_penduduk'].sum().reset_index()
...
with st.container(border=True):
    st.info(f"Jumlah Penduduk di Kecamatan {pilihkec} menurut Kelompok Umur, Semester {pilihsem} Tahun {pilihtahun}")
    ...
    if pilihkec and pilihtahun and pilihsem:
        ...
        with kol1d:
            pie_kk = px.pie(tabel, values='jumlah_penduduk', names='kategori_umur', 
        ...
        with kol1e:
            ...
            tabeljk = tabeljk[['kategori_umur', 'jumlah_penduduk']].sort_values(by='kategori_umur', ascending=True)
            bar_kk = px.bar(tabeljk, x='kategori_umur', y='jumlah_penduduk', color='kategori_umur', 
                            color_discrete_sequence=warna_options[pilihwarna])
            bar_kk.update_layout(showlegend=False)
            with st.container(border=True):
                st.plotly_chart(bar_kk, use_container_width=True)
        with kol1f:
            #tabeljk2 = tabeljk[['namakel', 'LAKI-LAKI', 'PEREMPUAN', 'Jumlah']]
    ...
    with kol2a:
        trimep = px.treemap(dataterpilih, path=['namakec', 'kategori_umur'], 
                            values='jumlah_penduduk', color_discrete_sequence=warna_options[pilihwarna])
        with st.container(border=True):
            st.plotly_chart(trimep, use_container_width=True)
    with kol2b:
        bulet = px.sunburst(dataterpilih, path=['namakec', 'kategori_umur'], values='jumlah_penduduk', 
                            color_discrete_sequence=warna_options[pilihwarna])
        with st.container(border=True):
    ...
    tren_kk = datagender[(datagender['namakec'] == pilihkec) & (datagender['tahun'] > 2018)]
    area_kk = px.area(tren_kk, x='sem', y='jumlah_penduduk', color='kategori_umur', 
                            color_discrete_sequence=warna_options[pilihwarna])
    st.plotly_chart(area_kk, use_container_width=True)
    
    st.subheader("", divider='rainbow')
    barkk = px.bar(tren_kk, x='sem', y='jumlah_penduduk', color='kategori_umur', 
                            color_discrete_sequence=warna_options[pilihwarna])
    st.plotly_chart(barkk, use_container_width=True)
    
    st.subheader("", divider='rainbow')
    line_kk = px.line(tren_kk, x='sem', y='jumlah_penduduk', color='kategori_umur', 
                            color_discrete_sequence=warna_options[pilihwarna])
    st.plotly_chart(line_kk, use_container_width=True)
    
    
st.link_button("Sumber Data", url="https://opendata.bandung.go.id/dataset/jumlah-penduduk-kota-bandung-berdasarkan-kelompok-umur")