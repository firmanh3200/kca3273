...
data = pd.read_excel("data/penduduk/jumlah_penduduk_golongan_darah.xlsx")
sort_data = data.sort_values(by=['tahun', 'semester', 'namakec', 'namakel', 'jumlah_penduduk'], ascending=[False,False,True,True, False])
#pivot_tabel = sort_data.pivot_table(index=['namakec', 'namakel', 'tahun', 'semester'], columns='tipe_goldar', values='jumlah_penduduk').reset_index()
#pivot_tabel = pivot_tabel.sort_values(by=['tahun', 'semester'], ascending=False)        
datagender = sort_data.groupby(['namakec', 'tipe_goldar', 'tahun', 'semester'])['jumlah_penduduk'].sum().reset_index()
...
with st.container(border=True):
    st.info(f"Jumlah Penduduk di Kecamatan {pilihkec} menurut Tipe Golongan Darah, Semester {pilihsem} Tahun {pilihtahun}")
    ...
    if pilihkec and pilihtahun and pilihsem:
        ...
        with kol1d:
            pie_kk = px.pie(tabel, values='jumlah_penduduk', names='tipe_goldar', 
        ...
        with kol1e:
            bar_kk = px.bar(tabel, x='tipe_goldar', y='jumlah_penduduk', color='tipe_goldar', 
                            color_discrete_sequence=warna_options[pilihwarna])
    ...
    with kol2a:
        trimep = px.treemap(dataterpilih, path=['namakel', 'tipe_goldar'], 
                            values='jumlah_penduduk', color_discrete_sequence=warna_options[pilihwarna])
        with st.container(border=True):
            st.plotly_chart(trimep, use_container_width=True)
    with kol2b:
        bulet = px.sunburst(dataterpilih, path=['namakec', 'namakel', 'tipe_goldar'], values='jumlah_penduduk')
        with st.container(border=True):
...
st.link_button("Sumber Data", url="https://opendata.bandung.go.id/dataset/jumlah-penduduk-kota-bandung-berdasarkan-golongan-darah-2")