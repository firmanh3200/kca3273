...
data = pd.read_excel("data/penduduk/jumlah_penduduk_kawin.xlsx")
sort_data = data.sort_values(by=['tahun', 'semester', 'namakec'], ascending=[False,False,True])
pivot_tabel = sort_data.pivot_table(index=['namakec', 'tahun', 'semester'], columns='status_kawin', values='jumlah_penduduk').reset_index()
pivot_tabel = pivot_tabel.sort_values(by=['tahun', 'semester'], ascending=False)        
datagender = sort_data.groupby(['namakec', 'status_kawin', 'tahun', 'semester'])['jumlah_penduduk'].sum().reset_index()
...
with st.container(border=True):
    st.info(f"Jumlah Penduduk di Kecamatan {pilihkec} menurut Status Perkawinan, Semester {pilihsem} Tahun {pilihtahun}")
    ...
    if pilihkec and pilihtahun and pilihsem:
        ...
        with kol1d:
            pie_kk = px.pie(tabel, values='jumlah_penduduk', names='status_kawin', 
        ...
        with kol1e:
            bar_kk = px.bar(tabel, x='status_kawin', y='jumlah_penduduk', color='status_kawin', 
                            color_discrete_sequence=warna_options[pilihwarna])
        ...
        with kol1f:
            ...
            tabeljk = tabeljk[['status_kawin', 'jumlah_penduduk']]
    ...
    with kol2a:
        trimep = px.treemap(dataterpilih, path=['namakec', 'status_kawin'], 
                            values='jumlah_penduduk', color_discrete_sequence=warna_options[pilihwarna])
        with st.container(border=True):
            st.plotly_chart(trimep, use_container_width=True)
    with kol2b:
        bulet = px.sunburst(dataterpilih, path=['namakec', 'status_kawin'], values='jumlah_penduduk')
        with st.container(border=True):
    ...
    area_kk = px.area(tren_kk, x='sem', y='jumlah_penduduk', color='status_kawin', 
                            color_discrete_sequence=warna_options[pilihwarna])
    st.plotly_chart(area_kk, use_container_width=True)
    
    st.subheader("", divider='rainbow')
    barkk = px.bar(tren_kk, x='sem', y='jumlah_penduduk', color='status_kawin', 
                            color_discrete_sequence=warna_options[pilihwarna])
    st.plotly_chart(barkk, use_container_width=True)
    
    st.subheader("", divider='rainbow')
    line_kk = px.line(tren_kk, x='sem', y='jumlah_penduduk', color='status_kawin', 
                            color_discrete_sequence=warna_options[pilihwarna])
